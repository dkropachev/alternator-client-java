...
default and may add application-specific headers. A custom whitelist must be non-null, non-empty,
contain only non-null and non-empty header names, and contain the complete required set for the
final configuration. The machine-readable default is recorded in
...
Both forms must produce the same filtered header map. A blocking wrapper must preserve the body
provider and metric collector. A non-blocking wrapper must preserve the body publisher, response
handler, duplex flag, metric collector, HTTP execution attributes, returned completion object, and
...
- A wrapper reports the same transport name as its delegate and propagates the delegate's completion
...
null, empty, malformed, or incomplete.
