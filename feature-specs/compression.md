...
be non-empty and contain no null entries. Duplicate values are removed while preserving their first
occurrence. Explicit disablement uses the dedicated disabled configuration rather than an empty
enabled list. Canonical machine-readable defaults are recorded in
...
Asynchronous decompression must obey demand, error, cancellation, single-emission, and completion
rules stated in this specification.
